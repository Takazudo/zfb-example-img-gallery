...
- Enhances the server-rendered 24-item pages with progressive infinite loading: a visible loading/error/end status, an automatic IntersectionObserver path, and a manual `Load next … photos` path. In healthy observer mode the canonical link remains in the feed but is hidden while one card-sized blank loader occupies the next grid slot; its spinner appears only when that tile enters view and starts loading, appended cards fade in, and a dormant loader moves to the new tail. The link becomes visible again for manual, no-observer, and observer-failure fallbacks. Each history entry keeps a bounded expanded-gallery snapshot so router navigation and Back can restore it; the canonical `/page/[page]` links remain the no-JavaScript fallback.
...
The first server-rendered page always contains up to 24 cards and a canonical next-page link. With JavaScript, the controller can append the next page while keeping the current URL; the one blank card-sized tile at the grid tail is both the visual loader and the IntersectionObserver target. Its centered spinner stays hidden until the tile intersects, then successful batches fade in before a fresh dormant tile is observed at the new tail. An automatic load remains limited to one request per leave/re-enter intersection or explicit downward continuation. The healthy automatic path hides the canonical link; manual loading remains visible when IntersectionObserver is unavailable or fails to initialize, and becomes the retry after a failed request. Loading, one-shot non-success, retry, and terminal `All photos loaded` states are announced in the feed's live region. Router history stores only bounded per-entry markup snapshots, so a photo navigation followed by Back can restore the loaded cards and scroll position without requiring a full reload. Disabling JavaScript follows the ordinary `/page/[page]` link and renders that page directly.
