...
export function createOgRoute(
  generation: string,
  renderer: OgCardRenderer,
): () => Promise<Response> {
  return async function OgCardRoute(): Promise<Response> {
    ...
    try {
      const hit = await env.BUCKET.get(ogObjectKey(photoId, generation));
      if (hit) return cardResponse(await hit.arrayBuffer(), request, hit.httpEtag);
      return cardResponse(
        await ensureOgCard(env, photoId, photo.r2_key, generation, renderer),
        request,
      );
    } catch {
